...
const finalCode = eval(`function edit() { \n${codeBlock}\n readMe.status = "${status.message} - ${activity.message}"; readMe.since = "${`${status.message} - ${activity.message}` == readMe.status ? `${dayjs().tz("Asia/Taipei").format("DD/MM/YY")}` : `${dayjs().tz("Asia/Taipei").format("DD/MM/YY")}`}"; readMe.location = "${location}"; readMe.stats = () => { return "${followers} followers with ${stargazers_count} stars on this repository, ${dayjs().tz("Asia/Taipei").format("DD/MM/YY")}" };  return readMe }; edit()`);
