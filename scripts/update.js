...
function stringify(code){ return Object.entries(code).reduce((a, e) => { if (typeof e[1] == "string") { a += `${e[0]}: "${e[1]}", `; }else if(typeof e[1] == "object") { a += `${e[0]}: ${stringify(e[1])}, `; }else { a+= `${e[0]}: ${e[1].toString()}`} return a;}, "{").slice(1, -2) + "}`"; }
fs.writeFileSync('./README.md', `${readMe.replace(codeBlock, `\n${js_beautify(`const readMe = ${code(finalCode)}`, { indent_size: 2, space_in_empty_paren: true })}\n`)}`);
})();
