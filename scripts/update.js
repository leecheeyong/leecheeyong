...
const finalCode = eval(`function edit() { \n${codeBlock}\n readMe.status = "${status.message} - ${activity.message}"; readMe.location = "${location}"; readMe.stats = () => { return "${followers} followers, ${stargazers_count} stars on ${dayjs().tz("Asia/Taipei").format("DD-MM-YY")}" };  return readMe }; edit()`);
