...
function stringify(code){ return Object.entries(code).reduce((a, e) => { if (typeof e[1] == "string") { a += `${e[0]}: "${e[1]}",`; }else if(typeof e[1] == "object") { a += `${e[0]}: ${JSON.stringify(e[1])},`; }else { a+= `${e[0]}: ${e[1].toString()}`} return a;}, "{").slice(1, -2) + "}"; }
