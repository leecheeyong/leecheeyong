...
const code = JSON.stringify(substrings.getOne('const readMe =', ';', codeBlock));
...
console.log(`${status.message} - ${activity.message}` != `${code.status}`, `${status.message} - ${activity.message}`, `${code.status}`, typeof code);
