...
function str(obj) {return Object.entries(obj).reduce((a, e) => {if(Array.isArray(a[1])){a += `${e[0]}: ${JSON.stringify(e[1])}, `;}else if(typeof e[1] == "object" && !Array.isArray(a[1])) {a += `${e[0]}: ${objify(e[1])}, `;}  else if (typeof e[1] == "function") {  a += `${e[0]}: ${e[1].toString()}, `; }else { a += `${e[0]}: "${e[1]}", `;} return a;}, "`{").slice(1, -2) + "}";}
